...
```{r}
valueBox(
  value = paste(format(round(sum((df$confirmed*100)/11),0), big.mark = "."), "", sep = " "),
  caption = "Projeção de total de casos",
...
plotly::plot_ly(data = df_daily) %>%
  ...
  plotly::add_trace(
    x = ~date,
    y = ~paste(format(round(projection_cum),0),big.mark = "."),
    type = "scatter",
    mode = "lines+markers",
    name = "Projeção",
    line = list(color = projection_color),
    marker = list(color = projection_color)
  ) %>%
  plotly::add_trace(
    x = ~date,
    y = ~death_cum,
...
pal <- colorFactor(c("orange", "red", "green"), domain = c("confirmed", "death"))
